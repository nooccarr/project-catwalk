...
let ExpandedView = ({expandedView, expandedViewZoom, trackAction, moduleName, mainImageWidth, mainImage, currentStyle,toggleExpandedViewZoom, slideLeft, slideRight, updateMainImage}) => {
  return (
    <div>
...
let ExpandedViewZoom = ({mainImageWidth, trackAction, moduleName, currentStyle, mainImage, onMouseMove, toggleExpandedView, toggleExpandedViewZoom, x, y}) => {
    return (
      <div>
        <div
          className='expandedViewZoom'
          style={{width: Math.min(mainImageWidth, (window.innerWidth - 60)*.7)}}
        >
          <img
            src={currentStyle.photos[mainImage].url}
...
let ProductInfo = ({category, name, default_price, original_price, sale_price, average_rating, trackAction, moduleName}) => {
  if (original_price === '') {
    ...
    return (
      <div onClick={(e) => trackAction(e, moduleName)}>
        <div className='star-reviews-wrapper'>{Stars(50, average_rating)}</div>
        <div
          id='read-all-reviews'
          onClick={()=>{document.getElementById('RatingAndReviews').scrollIntoView()}
        }
        >
          Read All Reviews
        </div>
        <div id='category'> {category} </div>
        <div id='expandedProductName'> {name} </div>
        <div id='price'> {original_price - sale_price === 0 ?
          original_price :
          <span>
...
let Test = ({name, trackAction}) => {
 return (
   <div id='test' onClick={(e)=>trackAction(e, name)}>Hi {name}</div>
 )
}
